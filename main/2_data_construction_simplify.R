...
### main ###
date(); d_num <- 0
main_df <- matrix(0, nrow = length(drivers)*200, ncol = 172, dimnames = list(NULL, NULL))
...
#         ex_turn <- length(which(turn_speed>=13.9))/length(which(cur<=100))
        ...
        # df
        main_df[d_num,] <- c(driver,trip,t(feature_speed),sd_speed,t(feature_tanAcc),t(feature_norAcc),
                            t(feature_totAcc), t(feature_curvature),sd_tanAcc,sd_norAcc,sd_cur,sd_totAcc,
                            t(feature_acceleration),t(feature_dec),sd_acc,sd_dec,t(feature_turn_sp),turn_point_mean,
                            sd_turn_sp,target)
...
names(main_df) <- c('driver','trip',names(feature_speed),'sd_speed',names(feature_tanAcc),names(feature_norAcc),
                    names(feature_totAcc), names(feature_curvature),'sd_tanAcc','sd_norAcc','sd_cur','sd_totAcc',
                    names(feature_acceleration),names(feature_dec),'sd_acc','sd_dec',names(feature_turn_sp),'turn_point_mean',
                    'sd_turn_sp','target')

save(main_df, file='data/main_df_172_median_smooth.RData')
