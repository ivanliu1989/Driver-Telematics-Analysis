...
svm <- data.frame(fread('submission_svmLinear_182.csv',header = T, stringsAsFactor = F))
gbm <- data.frame(fread('submission_gbm_182.csv',header = T, stringsAsFactor = F))
...
ensem_prob <- (svm[,2] + gbm[,2]  + rf[,2])/3 # + rf2[,2] + knn[,2] + nnet[,2]  + lg[,2]
# ensem_prob <- 4/(1/svm[,2] + 1/gbm[,2] + 1/nb[,2] + 1/rf[,2])

ensemble <- data.frame(driver_trip=svm[,1], prob=ensem_prob)
head(ensemble)

write.csv(ensemble, file = 'submissions182.csv', quote = F, row.names = F)
