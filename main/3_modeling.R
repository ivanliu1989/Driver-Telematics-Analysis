...
#     currentData$target <- 'Yes'
    ...
    g <- train(x = data.matrix(train[,c(features)]), y = as.factor(train$target), method = model,trControl = fitControl, 
               tuneLength = 6,metric = "ROC",tuneGrid = gbmGrid, preProc = c("center", "scale","pca"))#,expoTrans,repeats = 15, trace = FALSE)
...
write.csv(submission, file = 'submission_svmLinear_176_semi.csv', quote = F, row.names = F)
