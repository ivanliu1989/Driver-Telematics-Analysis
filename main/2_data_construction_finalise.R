...
for (driver in drivers){
    for (trip in trips){
        ...
        tp2 <- which(cur[,3] <= 100) #cur points
        tanAcc <- calcTangAccel(trip_data,2)[rp][tp2] #tangential acceleration
        norAcc <- calcNormAccel(speed[tp2],cur[tp2,3]) #normal acceleration
